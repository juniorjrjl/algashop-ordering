...
public interface OrderPersistenceEntityRepository extends JpaRepository<OrderPersistenceEntity, Long> {
    ...
    List<OrderPersistenceEntity> placedByCustomerInYear(
            @Param("customerId") final UUID customerId,
    ...
    @Query("""
        SELECT COALESCE(SUM(o.totalAmount), 0)
         ...
         WHERE o.customer.id = :customerId
           ...
           AND o.paidAt IS NOT NULL
    """)
    BigDecimal totalSoldForCustomer(@Param("customerId") final UUID customerId);

    @Override
    @EntityGraph(attributePaths = {"customer", "items"})
    Optional<OrderPersistenceEntity> findById(final Long orderId);
