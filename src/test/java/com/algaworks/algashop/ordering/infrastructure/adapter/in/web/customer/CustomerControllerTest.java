...
import com.algaworks.algashop.ordering.core.domain.model.customer.CustomerEmailInUseException;
import com.algaworks.algashop.ordering.core.domain.model.customer.CustomerNotFoundException;
import com.algaworks.algashop.ordering.infrastructure.adapter.in.web.customer.CustomerController;
...
import com.algaworks.algashop.ordering.utility.databuilder.presentation.CustomerUpdateInputJsonDataBuilder;
...
class CustomerControllerTest {
    ...
    @MockitoBean
    private ForManagingCustomer applicationService;
    @MockitoBean
    private ForQueryingCustomer queryService;
    @MockitoBean
    private ForQueryingShoppingCart shoppingCartQueryService;
    ...
    @Nested
    class CustomerControllerTestCreate {
    ...
    @Nested
    class CustomerControllerTestUpdate {

        @Test
        void updateCustomerContract() {
            final var id = UUID.randomUUID();
            doNothing().when(applicationService).update(any(UUID.class), any(CustomerUpdateInput.class));
            final var output = CustomerOutputDataBuilder.builder().build();
            final var addressOutput = output.getAddress();
            when(queryService.findById(any(UUID.class))).thenReturn(output);
            final var jsonInput = CustomerUpdateInputJsonDataBuilder.builder().build();
            RestAssuredMockMvc
                .given()
                    .accept(APPLICATION_JSON_VALUE)
                    .body(jsonInput.toString())
        ...
        private static final List<Arguments> updateCustomerErrorContract =
                List.of(
                        Arguments.of(
                                new CustomerEmailInUseException(),
                                CONFLICT,
        ...
        @ParameterizedTest
        @FieldSource
        void updateCustomerErrorContract(final Throwable throwable,
                                         final HttpStatus expectedStatus,
                                         final String expectedType) {
            doThrow(throwable).when(applicationService).update(any(UUID.class), any(CustomerUpdateInput.class));
            final var jsonInput = CustomerInputJsonDataBuilder.builder()
                    .build();
            RestAssuredMockMvc
                .given()
                    ...
                    .body(jsonInput.toString())
                .when()
                    .put("/api/v1/customers/{id}", UUID.randomUUID())
                .then()
                    .assertThat()
                    .contentType(APPLICATION_PROBLEM_JSON_VALUE)
                    .status(expectedStatus)
        ...
        @Test
        void updateCustomerErrorRequestContract() {
            final var jsonInput = CustomerUpdateInputJsonDataBuilder.builder()
                    .withFirstName(() -> null)
                    .withLastName(() -> null)
                    .build();
            RestAssuredMockMvc
    ...
    @Nested
    class CustomerControllerTestDelete{

        @Test
        void deleteCustomerContract() {
            doNothing().when(applicationService).archive(any(UUID.class));
            RestAssuredMockMvc
                .given()
                .when()
        ...
        private static final List<Arguments> deleteCustomerErrorContract =
                List.of(
                        Arguments.of(
                                new DomainException(),
                                UNPROCESSABLE_CONTENT,
        ...
        @ParameterizedTest
        @FieldSource
        void deleteCustomerErrorContract(final Throwable throwable,
                                         final HttpStatus expectedStatus,
                                         final String expectedType) {
            doThrow(throwable).when(applicationService).archive(any(UUID.class));
            RestAssuredMockMvc
                .given()
                .when()
                    .delete("/api/v1/customers/{id}", UUID.randomUUID())
    ...
    @Nested
    class CustomerControllerTestFilter {
    ...
    @Nested
    class CustomerControllerTestFindById {
